...
const initSocket = () => {
  ...
  socket.value.on("disconnect", (reason) => {
    ...
    if (reason === "io server disconnect") {
      ...
      updateSmartStatus(
        "error",
        "Server disconnected. Please refresh the page.",
        0
      );
    } else {
  ...
  socket.value.on("new-message", (message) => {
    ...
    if (
      ...
      message.targetUserId === currentUser.value?.userId
    ) {
      const speechOptions = {
...
const updateOtherUser = () => {
  if (room.value && currentUser.value) {
    ...
    otherUser.value = users.find(
      (user) => user.socketId !== currentUser.value.socketId
    );
  }
};
...
const initSpeechRecognition = () => {
  if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
    ...
    recognition.value.onnomatch = () => {
      ...
      sendStatusUpdate({
        type: "no-match",
        message: "No speech recognized, try again",
      });
    };
  } else {
...
const initWebRTC = async () => {
  try {
    ...
    const processedStream =
      audioProcessing.processedStream || localStream.value;
      ...
      reports.forEach((report) => {
        if (report.type === "inbound-rtp" && report.mediaType === "audio") {
          ...
          if (lossRate > 0.05 && audioQuality.value === "high") {
            ...
            console.log(
              "📈 Good connection detected, increasing audio quality"
            );
...
const processVoiceMessage = async (text, confidence, alternatives = []) => {
  ...
  if (confidence < 0.3) {
    ...
    sendStatusUpdate({
      type: "low-confidence",
      message: "Low confidence, try speaking clearer",
    });
  ...
  try {
    ...
    while (translationAttempts < maxAttempts) {
      try {
        ...
        translationAttempts++;
        console.warn(
          `Translation attempt ${translationAttempts} failed:`,
          error
        );
...
const detectLanguage = async (text) => {
  ...
  if (fullLangCode && fullLangCode !== userLanguage.value) {
    console.log(
      `🌍 Language detected: ${fullLangCode} (different from selected: ${userLanguage.value})`
    );
  }
...
const setInputVolume = (volume) => {
  ...
  if (window.audioGainNode) {
    ...
    window.audioGainNode.gain.setValueAtTime(
      gain,
      window.audioGainNode.context.currentTime
    );
...
const toggleNoiseReduction = () => {
  ...
  console.log(
    `🔇 Noise reduction ${noiseReduction.value ? "enabled" : "disabled"}`
  );
...
const toggleEchoCancellation = () => {
  ...
  console.log(
    `📢 Echo cancellation ${echoCancellation.value ? "enabled" : "disabled"}`
  );
...
const autoAdjustAudio = () => {
  ...
  console.log(
    `🤖 Auto-adjusted audio for ${connectionQuality.value} connection`
  );
};
  ...
  for (const api of availableAPIs) {
    try {
      ...
      if (translation && !translation.includes("[Translation unavailable")) {
        ...
        console.log(
          `✅ ${api.name} translation: "${translation}" (quality: ${qualityScore})`
        );
...
// MyMemory API implementation with rate limiting protection
const translateWithMyMemory = async (text, fromCode, toCode) => {
  // Check if we've hit rate limit recently
  const rateLimitKey = "mymemory_rate_limit";
  const lastRateLimit = localStorage.getItem(rateLimitKey);
  if (lastRateLimit && Date.now() - parseInt(lastRateLimit) < 3600000) {
    // 1 hour cooldown
...
// LibreTranslate API implementation with multiple endpoints and CORS handling
const translateWithLibreTranslate = async (text, fromCode, toCode) => {
  // Try multiple LibreTranslate instances to avoid CORS issues
  const endpoints = [
  ...
  for (const url of endpoints) {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `LibreTranslate endpoint ${url} returned ${response.status}`
        );
      ...
      console.warn(`LibreTranslate endpoint ${url} failed:`, error.message);
      continue; // Try next endpoint
    }
  }
...
// Enhanced fallback dictionary with comprehensive translations
const translateWithDictionary = async (text, fromCode, toCode) => {
  ...
  // If no translation found, try to provide a helpful fallback
  if (text.length < 50) {
    // Only for short phrases
    return `[${text}]`; // Return original text in brackets to indicate untranslated
  }
...
const persistentCache = {
  ...
  save() {
    try {
      const cacheData = Array.from(translationCache.entries()).map(
        ([key, value]) => [key, value]
      );
...
const calculateTranslationQuality = (
  translation,
  baseQuality,
  responseTime
) => {
...
const getTranslationFromCache = (key) => {
  ...
  if (cached) {
    ...
    const maxAge =
      cached.uses > 5 ? 7 * 24 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
...
const translationStats = {
  ...
  getSuccessRate() {
    return this.totalTranslations > 0
      ? (this.successfulTranslations / this.totalTranslations) * 100
      : 0;
  },
...
const translationOptimizer = {
  // Pre-translate common phrases with rate limiting protection
  async preloadCommonPhrases() {
    const commonPhrases = ["hello", "hi", "thank you", "goodbye", "yes", "no"];

    // Check if we should skip preloading due to recent rate limits
    const rateLimitKey = "mymemory_rate_limit";
    const lastRateLimit = localStorage.getItem(rateLimitKey);
    if (lastRateLimit && Date.now() - parseInt(lastRateLimit) < 3600000) {
      console.log("⏸️ Skipping preload due to rate limit cooldown");
      return;
    }

    // Only preload essential phrases using dictionary to avoid API calls
    ...
    for (const pair of essentialPairs) {
      for (const phrase of commonPhrases.slice(0, 3)) {
        // Only first 3 phrases
        const cacheKey = `${pair.from}-${pair.to}-${phrase.toLowerCase()}`;
        if (!getTranslationFromCache(cacheKey)) {
          try {
            // Use dictionary first to avoid API calls during preload
            const translation = await translateWithDictionary(
              phrase,
              pair.from,
              pair.to
            );
            ...
            // Ignore dictionary failures, don't call external APIs during preload
          }
        }
      }
    }
    ...
    console.log(
      `🚀 Preloaded ${preloadCount} essential phrases from dictionary`
    );
  },
...
const searchMessages = (query) => {
  ...
  const results = messages.value.filter((message) => {
    ...
    if (
      ...
      message.translatedText.toLowerCase().includes(searchText)
    ) {
      return true;
    }
    ...
    if (
      message.user?.name &&
      message.user.name.toLowerCase().includes(searchText)
    ) {
      return true;
    }
  ...
  switch (filterType) {
    ...
    case "errors":
      filtered = filtered.filter(
        (msg) =>
          msg.type === "error" || (msg.confidence && msg.confidence < 0.5)
      );
      break;
    case "all":
    default:
...
const getFilteredAndSortedMessages = () => {
  if (
    chatInterface.value.searchResults.length > 0 &&
...
const getConversationStats = () => {
  const stats = {
    ...
    originalMessages: messages.value.filter((m) => m.type === "original")
      .length,
  ...
  if (messages.value.length > 0) {
    ...
    if (confidenceMessages.length > 0) {
      stats.averageConfidence =
        confidenceMessages.reduce((sum, m) => sum + m.confidence, 0) /
...
const detectNetworkType = () => {
  if ("connection" in navigator) {
    ...
    if (connection) {
      connectionOptimization.value.networkType =
        connection.effectiveType || "unknown";
      console.log(
        `📡 Network type detected: ${connectionOptimization.value.networkType}`
      );
...
const monitorBandwidth = async () => {
  ...
  try {
    ...
    console.log(
      `📊 Bandwidth check: ${latency}ms latency, stability: ${connectionOptimization.value.connectionStability}`
    );
  } catch (error) {
...
const attemptReconnection = async () => {
  ...
  if (
    ...
    connectionOptimization.value.maxReconnectAttempts
  ) {
  ...
  const delay =
    connectionOptimization.value.reconnectDelay * Math.pow(2, attempt - 1); // Exponential backoff

  updateSmartStatus(
    "warning",
    `Reconnecting... (attempt ${attempt}/${connectionOptimization.value.maxReconnectAttempts})`,
...
const optimizeConnectionSettings = () => {
  ...
  if (peerConnection.value) {
    ...
    console.log(
      "⚙️ Optimized WebRTC configuration for",
      stability,
      "connection"
    );
  }
...
const handleCriticalError = (error) => {
  ...
  showUserFriendlyError(
    error,
    "A critical error occurred. The application may not function properly. Please refresh the page."
  );
};
...
const attemptErrorRecovery = async (error) => {
  ...
  try {
    ...
    handleError(recoveryError, ErrorTypes.UNKNOWN, ErrorSeverity.HIGH, {
      originalError: error,
    });
  }
};
...
const recoverFromTranslationError = async (error) => {
  ...
  if (context.text && context.fromLang && context.toLang) {
    try {
      ...
      const fallbackTranslation = await translateWithDictionary(
        context.text,
        context.fromLang,
        context.toLang
      );
      ...
      updateSmartStatus(
        "warning",
        "Translation limited. Some features may not work.",
        0
      );
    }
  }
...
const enableGracefulDegradation = (errorType) => {
  switch (errorType) {
    case ErrorTypes.AUDIO:
      ...
      updateSmartStatus(
        "warning",
        "Audio features disabled. Using text-only mode.",
        0
      );
      break;
    case ErrorTypes.TRANSLATION:
      ...
      updateSmartStatus(
        "warning",
        "Advanced translation disabled. Using basic translation.",
        0
      );
      break;
    case ErrorTypes.NETWORK:
...
const enableOfflineMode = () => {
  ...
  updateSmartStatus(
    "warning",
    "Offline mode enabled. Limited functionality.",
    0
  );
...
const showPermissionGuide = (permission) => {
  ...
  const message =
    guides[permission] || "Please check your browser permissions.";
...
const showBrowserCompatibilityWarning = (compatibility) => {
  const message = `Your browser doesn't support: ${compatibility.unsupportedFeatures.join(
    ", "
  ...
  if ("getEntriesByType" in performance) {
    setInterval(() => {
      ...
      if (networkEntries.length > 0) {
        ...
        performance.value.metrics.networkRequests =
...
const checkForMemoryLeaks = () => {
  ...
  if (trend > 50 * 1024 * 1024) {
    ...
    handleError(
      new Error("Memory usage increasing rapidly"),
      ErrorTypes.BROWSER,
...
const getPerformanceReport = () => {
  const report = {
    ...
    memory: {
      ...
      peak: Math.max(
        ...performance.value.monitoring.memoryChecks.map((c) => c.used)
      ),
      average:
        performance.value.monitoring.memoryChecks.reduce(
          (sum, c) => sum + c.used,
          0
        ) / performance.value.monitoring.memoryChecks.length,
    },
    cache: {
...
const runBrowserCompatibilityTests = () => {
  ...
  Object.entries(tests).forEach(([browser, test]) => {
    try {
      ...
      console.log(
        `${
          testing.value.browserCompatibility[browser] ? "✅" : "❌"
...
const optimizeForMobile = () => {
  ...
  if (window.innerWidth <= 768) {
    ...
    if (
      connectionOptimization.value.networkType === "3g" ||
      connectionOptimization.value.networkType === "2g"
    ) {
      setAudioQuality("standard");
    }
  }
...
const runLoadTests = async () => {
  ...
  try {
    ...
    testing.value.loadTesting = {
      ...
      averageResponseTime:
        (testResults.connectionTime +
          testResults.messageLatency +
          testResults.translationSpeed) /
        3,
...
const runHealthCheck = async () => {
  ...
  try {
    ...
    healthStatus.overall =
      healthStatus.server && healthStatus.translation && healthStatus.webrtc;
  ...
  getCulturalHints(fromLang, toLang) {
    const culturalNotes = {
      ...
      "ja-JP": {
        ...
        notes:
          "Japanese communication values politeness and indirect expression",
      },
      "de-DE": {
        formality: "medium",
        directness: "high",
    ...
    return {
      from: culturalNotes[fromLang] || {
        formality: "medium",
        directness: "medium",
      },
      to: culturalNotes[toLang] || {
        formality: "medium",
        directness: "medium",
      },
    };
...
const enhanceTranslationWithContext = async (
  originalText,
  translation,
  fromLang,
  toLang
) => {
  const context = conversationContext.getContextForTranslation(
    originalText,
    fromLang,
    toLang
  );
  ...
  return {
    ...
    contextHints,
    confidence: calculateContextualConfidence(
      originalText,
      translation,
      context
    ),
...
const applyCulturalAdaptation = (translation, culturalHints, topics) => {
  ...
  if (
    culturalHints.to.formality === "high" &&
    culturalHints.from.formality === "casual"
  ) {
    adaptedTranslation = makeMoreFormal(adaptedTranslation);
  } else if (
    culturalHints.to.formality === "casual" &&
    culturalHints.from.formality === "high"
  ) {
...
const applyBusinessContext = (text) => {
  ...
  return text.replace(/\bmeet\b/gi, "meeting").replace(/\btalk\b/gi, "discuss");
};
...
const speakText = (text, language, options = {}) => {
  if (!synthesis.value || !text || text.includes("[Translation unavailable"))
    return;
...
const selectOptimalVoice = (
  ...
  let candidateVoices = voices.filter(
    (v) => v.lang.startsWith(langCode) || v.lang.startsWith(fullLangCode)
  );
...
<template>
  ...
  >
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div
        class="absolute -top-40 -right-40 w-80 h-80 bg-purple-500/10 rounded-full blur-3xl animate-float"
      ></div>
      <div
        class="absolute -bottom-40 -left-40 w-80 h-80 bg-blue-500/10 rounded-full blur-3xl animate-float-delayed"
      ></div>
      <div
        class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-indigo-500/5 rounded-full blur-3xl animate-pulse-slow"
      ></div>
    </div>

    <!-- Connection Status Bar -->
    <div
      class="relative z-10 bg-black/20 backdrop-blur-sm border-b border-white/10 px-4 py-2 transition-all duration-300"
    >
      <div class="flex items-center justify-between max-w-6xl mx-auto">
        <div class="flex items-center gap-3">
          ...
          <div
            v-if="room"
            class="flex items-center gap-4 ml-4 animate-slide-in"
          >
            <div class="flex items-center gap-2">
            ...
            >
              <div
                :class="[
                  ...
                  connectionHealth.status === 'excellent'
                    ? 'bg-green-400 animate-pulse'
                    : connectionHealth.status === 'good'
                    ? 'bg-blue-400 animate-pulse'
                    : connectionHealth.status === 'fair'
                    ? 'bg-yellow-400 animate-pulse'
                    : connectionHealth.status === 'poor'
              ...
              <span
                v-if="connectionHealth.latency > 0"
                class="text-xs text-white/50"
              >
                {{ connectionHealth.latency }}ms
              </span>
              <span
                v-if="connectionHealth.packetLoss > 0"
            ...
            <div
              v-if="smartStatus.type !== 'idle'"
              class="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-lg animate-fade-in"
            >
              <div
                :class="[
                  'w-2 h-2 rounded-full',
                  smartStatus.type === 'loading'
                    ? 'bg-blue-400 animate-spin'
                    : smartStatus.type === 'processing'
                    ? 'bg-yellow-400 animate-pulse'
                    : smartStatus.type === 'speaking'
              ...
              <div
                v-if="smartStatus.progress > 0"
                class="w-8 h-1 bg-white/20 rounded-full overflow-hidden"
              >
                <div
    ...
    <div class="flex-1 p-4">
      <div class="max-w-6xl mx-auto">
        ...
        >
          <div
            class="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8 w-full max-w-md shadow-2xl animate-fade-in-up"
          >
            <div class="text-center mb-8">
              <div class="text-6xl mb-4 animate-bounce-slow">🌍</div>
              <h1
                class="text-3xl font-bold text-white mb-3 bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent"
              >
                Real-Time Translation Chat
              </h1>
              <p class="text-white/60 text-lg">
                Connect with someone and chat in different languages
              </p>
              <div
                class="flex items-center justify-center gap-2 mt-4 text-white/40"
              >
                <div
            ...
            <div class="space-y-6">
              ...
              <div
                ...
                class="p-4 bg-red-500/20 border border-red-500/30 rounded-xl text-red-300 text-sm animate-shake flex items-center gap-2"
              >
                ⚠️ {{ connectionError }}
              </div>
              ...
              <!-- Action Buttons -->
              <div
                class="space-y-3 animate-slide-in"
                style="animation-delay: 0.4s"
              >
                <button
                  v-if="roomCode"
              ...
              <div
                class="text-center text-white/40 text-sm animate-fade-in"
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[85vh]">
          ...
          <div class="lg:col-span-1 space-y-4">
            <!-- Current User Card -->
            <div
              class="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl animate-fade-in-up hover:bg-white/10 transition-all duration-300"
            >
              <div class="text-center">
                <div class="relative inline-block">
                  <div class="text-4xl mb-3 animate-bounce-slow">
                    {{ currentUser?.avatar || "👤" }}
                  </div>
                  <div
                ...
                <p
                  class="text-sm text-white/60 flex items-center justify-center gap-1 mt-1"
                >
                ...
                <!-- User Status -->
                <div
                  class="mt-4 p-4 bg-gradient-to-r from-white/5 to-white/10 rounded-xl border border-white/10"
                >
                  <div class="flex items-center justify-center gap-2">
                    ...
                    >
                      <div class="relative">
                        ...
                        <div
                          class="absolute inset-0 w-3 h-3 bg-blue-400 rounded-full animate-ping"
                        ></div>
                      </div>
                    ...
                    <div
                      ...
                      class="flex items-center gap-2 text-yellow-400 animate-pulse"
                    >
                      <div class="relative">
                    ...
                    <div
                      ...
                      class="flex items-center gap-2 text-green-400 animate-pulse"
                    >
                      <div class="relative">
            ...
            >
              <div class="text-center">
                <div v-if="otherUser" class="space-y-3">
                  ...
                  <p
                    class="text-sm text-white/60 flex items-center justify-center gap-1"
                  >
                    {{ getLanguageFlag(otherUser.language) }}
                    {{ getLanguageName(otherUser.language) }}
                  </p>

                  <!-- Other User Status -->
                  <div
                    class="p-4 bg-gradient-to-r from-white/5 to-white/10 rounded-xl border border-white/10"
                  >
                    <div class="flex items-center justify-center gap-2">
                      <div
                        v-if="
                          userStatus[otherUser.userId]?.type === 'listening'
                        "
                        class="flex items-center gap-2 text-blue-400 animate-pulse"
                      >
                        <div class="relative">
                      ...
                      <div
                        v-else-if="
                          userStatus[otherUser.userId]?.type === 'processing'
                        "
                        class="flex items-center gap-2 text-yellow-400 animate-pulse"
                      >
                        <div class="relative">
                      ...
                      <div
                        v-else-if="
                          userStatus[otherUser.userId]?.type === 'speaking'
                        "
                        class="flex items-center gap-2 text-green-400 animate-pulse"
                      >
                        <div class="relative">
                ...
                <div v-else class="text-white/40 animate-pulse">
                  ...
                  >
                    ...
                    <span
                      class="font-mono text-white bg-white/10 px-2 py-1 rounded mt-1 inline-block"
                      >{{ room?.id }}</span
                    >
                  </div>
                </div>
            ...
            >
              <h4
                class="font-bold text-white mb-6 text-center text-lg flex items-center justify-center gap-2"
              >
                🎙️ Voice Controls
              </h4>
              ...
              <div class="flex justify-center mb-4">
                ...
                >
                  ...
                  <div
                    v-if="isListening"
                    class="absolute inset-0 bg-gradient-to-r from-red-400 to-red-600 animate-pulse"
                  ></div>

                  <!-- Ripple effect -->
                  <div
                    v-if="isListening"
                    class="absolute inset-0 border-4 border-red-300 rounded-full animate-ping"
                  ></div>
              ...
              <div class="text-center space-y-2">
                ...
                <div v-if="isListening" class="mt-3 space-y-2">
                  ...
                  <div class="flex items-center justify-center gap-1">
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 12px; animation-delay: 0s"
                    ></div>
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 20px; animation-delay: 0.1s"
                    ></div>
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                    ...
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 24px; animation-delay: 0.3s"
                    ></div>
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 18px; animation-delay: 0.4s"
                    ></div>
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 14px; animation-delay: 0.5s"
                    ></div>
                    <div
                      class="w-1 bg-blue-400 rounded-full animate-pulse"
                      style="height: 22px; animation-delay: 0.6s"
                    ></div>
                  </div>
                  ...
                  <div
                    v-if="currentMessage"
                    class="bg-blue-500/20 border border-blue-500/30 rounded-lg p-2 text-xs text-blue-300"
                  >
                ...
                <div
                  v-if="!otherUser"
                  class="text-xs text-yellow-400 bg-yellow-400/10 px-3 py-2 rounded-lg border border-yellow-400/20"
                >
                ...
                <div v-if="otherUser" class="mt-4 space-y-3">
                  <div class="flex items-center justify-center gap-3">
                    ...
                    >
                      <option value="high" class="bg-slate-800">
                        🔊 High Quality
                      </option>
                      <option value="standard" class="bg-slate-800">
                        📻 Standard
                      </option>
                    </select>
                  </div>
                ...
                <div
                  v-if="otherUser"
                  class="mt-4 space-y-3 border-t border-white/10 pt-4"
                >
                  <h5 class="text-sm font-medium text-white/70 text-center">
                    🎛️ Audio Controls
                  </h5>
                  ...
                  <!-- Audio Presets -->
                  <div class="space-y-2">
                    <label class="text-xs text-white/60 block text-center"
                      >🎚️ Presets</label
                    >
                  ...
                  <div class="mt-4 pt-3 border-t border-white/10">
                    <div class="flex items-center justify-between mb-2">
                      ...
                      >
                      <span class="text-xs text-white/40"
                        >{{ translationCache.size }} entries</span
                      >
                    </div>
                  ...
                  <div class="mt-4 pt-3 border-t border-white/10">
                    <h5
                      class="text-sm font-medium text-white/70 text-center mb-3"
                    >
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-2">
                          <span
                            :class="[
                              ...
                              connectionOptimization.networkType === '4g' ||
                              connectionOptimization.networkType === '5g'
                                ? 'bg-green-500/20 text-green-400'
                                : connectionOptimization.networkType === '3g'
                                ? 'bg-yellow-500/20 text-yellow-400'
                                : connectionOptimization.networkType === '2g' ||
                                  connectionOptimization.networkType ===
                                    'slow-2g'
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <button
                          @click="
                            connectionOptimization.adaptiveQuality =
                              !connectionOptimization.adaptiveQuality
                          "
                          :class="[
                      ...
                      <div class="flex items-center justify-between">
                        <span class="text-xs text-white/60"
                          >Bandwidth Monitor:</span
                        >
                        <button
                          @click="
                            connectionOptimization.bandwidthMonitoring =
                              !connectionOptimization.bandwidthMonitoring
                          "
                          :class="[
                            'w-8 h-4 rounded-full transition-all duration-300 relative',
                      ...
                      <!-- Reconnection Status -->
                      <div
                        v-if="connectionOptimization.isReconnecting"
                        class="text-xs text-center p-2 bg-yellow-500/20 rounded-lg border border-yellow-500/30"
                      >
                        <div
                          class="flex items-center justify-center gap-2 text-yellow-300"
                        >
                          <div
                          ...
                          <span
                            >Reconnecting... ({{
                              connectionOptimization.reconnectAttempts
                            }}/{{
                              connectionOptimization.maxReconnectAttempts
                            }})</span
                          >
                        </div>
                      </div>
                      ...
                      <button
                        v-if="
                          !isConnected && !connectionOptimization.isReconnecting
                        "
                      ...
                      >
                        ⚠️ {{ errorHandling.errorCount }} Error{{
                          errorHandling.errorCount > 1 ? "s" : ""
                        }}
                      </button>
                    </div>
                  </div>
                  ...
                  <!-- Error Panel -->
                  <div
                    v-if="errorHandling.showErrorPanel"
                    class="mt-4 pt-3 border-t border-white/10"
                  >
                    <div
                      class="bg-red-500/10 rounded-xl p-4 border border-red-500/30"
                    >
                      <div class="flex items-center justify-between mb-3">
                        <h5 class="text-sm font-medium text-red-300">
                          🚨 Error Log
                        </h5>
                        <button
                          @click="clearErrors"
                      ...
                      <div class="space-y-2 max-h-32 overflow-y-auto">
                        <div
                          v-for="error in errorHandling.errors.slice(-5)"
                          :key="error.id"
          ...
          <div
            class="lg:col-span-2 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 flex flex-col shadow-xl animate-fade-in-up"
            style="animation-delay: 0.3s"
          >
            <!-- Enhanced Chat Header -->
            <div
              class="border-b border-white/10 bg-gradient-to-r from-white/5 to-white/10"
            >
              <div class="p-6">
                <div class="flex items-center justify-between">
                  <h3
                    class="font-bold text-white text-xl flex items-center gap-2"
                  >
                    💬 Conversation
                    <span
                      v-if="messages.length > 0"
                      class="text-sm text-white/50 font-normal"
                    >
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    >
                      ...
                      <span
                        v-if="chatInterface.searchResults.length > 0"
                        class="text-xs"
                      >
                    ...
                    <div
                      v-if="messages.length > 0"
                      class="text-xs text-white/50 bg-white/5 px-3 py-2 rounded-lg"
                    >
                      <div class="flex items-center gap-3">
                        <span
                          >{{ getConversationStats().totalMessages }} msgs</span
                        >
                        <span
                          >{{
                            Math.round(
                              getConversationStats().averageConfidence * 100
                            )
                          }}% avg</span
                        >
                        <span>{{
                          formatDuration(
                            getConversationStats().conversationDuration
                          )
                        }}</span>
                      </div>
                    </div>
                    ...
                    <!-- Language Indicator -->
                    <div
                      class="flex items-center gap-3 bg-white/10 px-4 py-2 rounded-xl border border-white/20"
                    >
                      <div class="flex items-center gap-2">
                        <span class="text-lg">{{
                          getLanguageFlag(userLanguage)
                        ...
                        <span class="text-xs text-white/60">{{
                          userLanguage.split("-")[0].toUpperCase()
                        }}</span>
                      </div>
                      <div class="text-white/40 animate-pulse">↔️</div>
                      <div class="flex items-center gap-2">
                        <span class="text-lg">{{
                          otherUser ? getLanguageFlag(otherUser.language) : "🌍"
                        }}</span>
                        <span class="text-xs text-white/60">{{
                          otherUser
                            ? otherUser.language.split("-")[0].toUpperCase()
                            : "ANY"
                        }}</span>
                      </div>
                    </div>
              ...
              <!-- Search Panel -->
              <div
                v-if="chatInterface.showSearch"
                class="px-6 pb-4 animate-fade-in"
              >
                <div class="bg-white/5 rounded-xl p-4 border border-white/10">
                  <div class="flex items-center gap-3 mb-3">
                    <div class="flex-1 relative">
                      ...
                      />
                      <div
                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-white/30"
                      >
                        🔍
                      </div>
                    </div>
                  ...
                  <div class="flex items-center gap-3 text-sm">
                    <div class="flex items-center gap-2">
                      ...
                      >
                        <option value="all" class="bg-slate-800">
                          All Messages
                        </option>
                        <option value="original" class="bg-slate-800">
                          Original Only
                        </option>
                        <option value="translation" class="bg-slate-800">
                          Translations Only
                        </option>
                        <option value="errors" class="bg-slate-800">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <select
                        ...
                        class="bg-white/10 border border-white/20 rounded px-2 py-1 text-white text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
                      >
                        <option value="newest" class="bg-slate-800">
                          Newest First
                        </option>
                        <option value="oldest" class="bg-slate-800">
                          Oldest First
                        </option>
                        <option value="relevance" class="bg-slate-800">
                          Relevance
                        </option>
                      </select>
                    </div>
                    ...
                    <div
                      v-if="chatInterface.searchQuery"
            ...
            >
              ...
              >
                <div class="text-center">
                  ...
                  <p class="text-sm mt-2">
                    Your messages will be translated automatically
                  </p>
                </div>
              </div>
              ...
              <div
                v-for="message in getFilteredAndSortedMessages()"
                :key="message.id"
                :id="`message-${message.id}`"
                :class="[
                  'space-y-2 transition-all duration-300',
                  chatInterface.selectedMessage?.id === message.id
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between mt-2">
                      ...
                      <div class="flex items-center gap-2">
                        <span
                          v-if="message.confidence"
                          class="text-xs px-2 py-1 rounded-full bg-blue-500/20 text-blue-300"
                        >
                          {{ Math.round(message.confidence * 100) }}%
                        </span>
                        <span
                          v-if="
                            ...
                            message.detectedLanguage !== message.language
                          "
                ...
                >
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between mt-2">
                      ...
                      <div class="flex items-center gap-2">
                        <span
                          v-if="message.contextualConfidence"
                          class="text-xs px-2 py-1 rounded-full bg-green-500/20 text-green-300"
                        >
                          {{ Math.round(message.contextualConfidence * 100) }}%
                          context
                        </span>
                        <span
                          v-if="message.translationAttempts > 1"
                    ...
                    <div
                      v-if="
                        message.contextHints && message.contextHints.length > 0
                      "
                      class="mt-2 space-y-1"
                    >
                      <div
                        v-for="hint in message.contextHints"
                        :key="hint.type"
              ...
              <!-- Typing Indicator -->
              <div
                v-if="typingIndicator.isTyping"
                class="animate-fade-in px-4 pb-2"
              >
            ...
            <!-- Message Input -->
            <div
              class="p-6 border-t border-white/10 bg-gradient-to-r from-white/5 to-white/10"
            >
              ...
              <div
                class="flex items-center justify-center mt-4 text-sm text-white/40 space-x-4"
              >
                <div class="flex items-center gap-1">
                  <span>⌨️</span>
